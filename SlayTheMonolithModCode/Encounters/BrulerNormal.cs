...
public sealed class BrulerNormal : CustomEncounterModel, ILocalizationProvider
{
    public BrulerNormal() : base(RoomType.Monster) { }
    ...
    public List<(string, string)>? Localization => new EncounterLoc(
        Title: "Bruler",
        LossText: "Bested by the Bruler.");

    public override IEnumerable<MonsterModel> AllPossibleMonsters => new MonsterModel[]
    {
        ModelDb.Monster<Bruler>(),
    };

    protected override IReadOnlyList<(MonsterModel, string?)> GenerateMonsters() =>
        new List<(MonsterModel, string?)>
        {
            (ModelDb.Monster<Bruler>().ToMutable(), null),
        };
}
