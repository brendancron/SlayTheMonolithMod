...
public sealed class LampmasterBoss : CustomEncounterModel, ILocalizationProvider
{
    ...
    // 5-slot scene: Lampmaster pinned to "lampmaster" in the middle; lamp1-4
    // are the kill-order positions the ritual fills. BaseLib reads slot names
    // off this scene's Marker2D children.
    public override string? CustomScenePath =>
        "res://SlayTheMonolithMod/scenes/encounters/lampmaster_boss.tscn";

    public override IEnumerable<MonsterModel> AllPossibleMonsters => new MonsterModel[]
    {
        ModelDb.Monster<Lampmaster>(),
        ModelDb.Monster<Lamp>(),
    };

    protected override IReadOnlyList<(MonsterModel, string?)> GenerateMonsters() =>
        new List<(MonsterModel, string?)>
        {
            (ModelDb.Monster<Lampmaster>().ToMutable(), "lampmaster"),
        };
}
